import BstNode from './binary_tree_node';
import VisualTree from './visual_tree';
...
// '_existsInSubtree'
// NB: I do not take credit for the implementation of this algorithm.
// NB: Adapted from Bucket.js to ES6 with prototypical-inheritance.
class BinaryTree extends VisualTree{
  constructor() {
    this._root = null;
  }
  /**
  ...
  */
  ...
  reposition() {
    this._reposition(this._root, this.getHeight() - 1);
  }

  _reposition(node, height, depth = 0, idx = 1) {
    if(!node)
      return;

    const width = Math.pow(2, depth);
    const delta = Math.pow(2, height - depth + 1);
    const midIdx = idx - ((width + 1)/ 2);
    ...
    this._reposition(node._left, height, depth + 1, 2 * idx - 1);
    this._reposition(node._right, height, depth + 1, 2 * idx);
  }
}
